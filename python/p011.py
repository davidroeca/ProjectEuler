...
def line_to_num_list(line):
    str_list = re.split(r"[\s]", line.rstrip())
    return [int(s) for s in str_list]

def file_to_num_square(filename):
    lines = fileinput.input(filename)
    return [line_to_num_list(line) for line in lines]

def infer_size(square):
    return len(square)
...
def scan_row(square, row_index, n):
    max_value = 1
    max_index = n - 1
    ...
    for col_index in xrange(max_scan_start + 1):
        ...
        if row_index >= 3:
            max_value = max(max_value, diag_up_prod(square, row_index, col_index))
        max_value = max(max_value, horiz_prod(square, row_index, col_index))
    return max_value
